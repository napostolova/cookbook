...
const {    getAll, getMyItemsById, create, update, remove, like } = require('../services/recipe');
...
router.get(`/my-recipes/:id`, isAuth(), async(req, res) => {
...
router.post('/:id', isAuth(), preload(), async( req, res) => {
    ...
    try {
      const result = await like(recipeId, userId);
        res.status(200).json(result);
